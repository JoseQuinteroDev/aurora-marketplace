...
- **Security — auth hardening** (OWASP A07, `security/token/`): access tokens are short-lived (**15 min**); sessions are extended by **refresh-token rotation with reuse detection** — opaque, single-use, SHA-256-at-rest tokens (30-day) that rotate on `POST /api/auth/refresh`. Reuse outside a 10s grace window revokes the whole token family and denylists its access tokens (`TokenDenylistService`, checked per request; `POST /api/auth/revoke` is public so idle sessions can log out). Also: per-account **login lockout** (consecutive failures → 15-min lock), **password reset**, **email verification** (a verified email gates checkout / order placement), and **breached-password rejection** at register/reset via HIBP k-anonymity (`security/password/`, fail-open). Reset and verification are anti-enumeration (identical responses + a fixed latency floor). Roadmap: `docs/appsec/security-master-plan.md`.
- **Security — MFA (in progress on `feat/mfa`)**: the remaining Phase 1 item. So far only the **dependency-free crypto foundation** exists under `security/mfa/` — `TotpGenerator` (RFC 6238 TOTP, HMAC-SHA1, constant-time verify with a ±step skew window), `Base32` (RFC 4648 secret encoding), and `MfaSecretCipher` (AES-256-GCM authenticated encryption of the shared secret at rest, key from `app.security.mfa.encryption-key`, base64 16/24/32 bytes). **No DB columns, endpoints, or login-flow wiring yet** — see `docs/appsec/mfa-design.md` for the implementation-ready spec.
- **Security — concurrency & idempotency** (OWASP A04, "never trust the client" extended to races/retries): inventory/orders/payments carry a JPA `@Version` column (`V13`) so a concurrent lost update fails with `ObjectOptimisticLockingFailureException` → **HTTP 409** instead of silently overwriting; coupon redemption is atomic under a row lock; checkout accepts a client `Idempotency-Key` header bound to the produced order per-user via a `UNIQUE(user_id, idempotency_key)` constraint (`checkout_idempotency_keys`, `V14`) so a replayed/double-submitted checkout resolves to one order/charge. The frontend sends a stable key on checkout confirm (`checkout.service.ts`).
...
- **Schema is Flyway-managed** (`backend/src/main/resources/db/migration/V*__*.sql`) with `ddl-auto: validate` — never rely on Hibernate to create/alter tables; add a new `V#` migration instead. **Latest is `V14`**; the auth-hardening tables are `V7` (login lockout), `V8` (token denylist), `V10` (refresh tokens), `V11` (password-reset tokens), `V12` (email verification). The A04 concurrency/idempotency work is `V13` (optimistic-locking `@Version` columns on inventory/orders/payments) and `V14` (`checkout_idempotency_keys`). Spring Batch owns its metadata tables; Aurora adds `batch_job_audit` for admin visibility.
