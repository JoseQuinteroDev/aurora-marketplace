...
| `MfaServiceTest` + `MfaKeyValidatorTest` + `MfaControllerTest` | `backend/.../auth/service|controller/` + `security/mfa/` | TOTP MFA **enrollment** (opt-in, login not yet gated): enroll mints a 160-bit secret stored **encrypted** (AES-GCM) and pending (not enabled); confirm enables **only on a valid current TOTP**; disable requires a valid code and clears the secret; a wrong code → generic `MFA_INVALID_CODE`. `/api/auth/mfa/**` requires authentication (401 anonymous). The MFA encryption key fail-fasts under `prod` on a placeholder/blank/wrong-length value (mirrors `JwtSecretValidator`). (OWASP A07) |
